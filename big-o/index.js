...
// ANSWER:
// **No, this is running as fast as we need it to. Rewriting it would only make the time complexity longer**
...
function squareEveryDigit(number) {
    let sqFunc = num => Number(num);
    let sqArr = Array.from(String(number), sqFunc);
    const sqNums = sqArr.map(n => n * n);
    const solution = sqNums.join('');
    return + solution  
    };
  ...
  // found this online, but me and Avery went over both and understand the elements and also both are doing essentially the same thing.
  // this was a fun challenge, and one of the first that I was able to complete fully
